    ...
    def add_individual_entity_blocks(self, entities):
        ...
        for target_name in sorted_target_names:
            ...
            for entity in target_entities:
                ...
                description_para.add_run('\nAm ')
                run_date = description_para.add_run(str(entity['DATE']))
                run_date.bold = True
                
                description_para.add_run(' wurde ein Account auf der Plattform ')
                run_platform = description_para.add_run(str(entity['PLATFORM']))
                run_platform.bold = True
                description_para.add_run(' festgestellt. ')

                if entity['accountID'] is not None:
                    description_para.add_run('Die eindeutige Kennung (UID) ')
                    run_acc_id = description_para.add_run(str(entity['accountID']))
                    run_acc_id.bold = True
                    description_para.add_run(' konnte ermittelt werden. ')
                
                if entity['SCREENAME'] is not None:
                    description_para.add_run('Als Anzeigename (Screenname) hat der Nutzer ')
                    run_screen = description_para.add_run(str(entity['SCREENAME']))
                    run_screen.bold = True
                    description_para.add_run(' ausgewählt. ')

                if entity['VANITYNAME'] is not None:
                    description_para.add_run('Als Nutzername (Vanityname) wurde ')
                    run_vanity = description_para.add_run(str(entity['VANITYNAME']))
                    run_vanity.bold = True
                    description_para.add_run(' verwendet.')

                description_para.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY  # Blocksatz
                self.set_font_properties(description_para, is_bold=False)
                ...
                if has_details:
                    ...
                    def add_separator():
                        # Fügt nur ein Trennzeichen hinzu, wenn es nicht das erste Detail ist
                        nonlocal is_first_detail
                        if not is_first_detail:
                            details_para.add_run(" ")
                        is_first_detail = False
                    
                    if os.path.exists(image_path):
                        add_separator()
                        details_para.add_run("Profilbild: ")
                        run_image = details_para.add_run("siehe oben")
                        run_image.bold = True

                    if entity['EMAIL']:
                        add_separator()
                        details_para.add_run("E-Mail: ")
                        run_email = details_para.add_run(entity['EMAIL'])
                        run_email.bold = True

                    if entity['PHONENUMBER']:
                        add_separator()
                        details_para.add_run("Telefonnummer: ")
                        run_phone = details_para.add_run(entity['PHONENUMBER'])
                        run_phone.bold = True

                    if entity['PRIVATE']:
                        add_separator()
                        details_para.add_run("Privater Account: ")
                        run_private = details_para.add_run("Ja") # Abgeleiteter DB-Inhalt, fett
                        run_private.bold = True
                    
                    self.set_font_properties(details_para, is_bold=False)
    ...
    def add_platform_breakdown(self, entities):
        ...
        for platform, platform_entities in platforms.items():
            ...
            for i, entity in enumerate(platform_entities, 1):
                ...
                table = self.document.add_table(rows=1, cols=2)
                table.autofit = False
                table.columns[0].width = Inches(4.5) # Breite für Text
                table.columns[1].width = Inches(2.0) # Breite für Bild

                # Mache die Tabellenränder unsichtbar
                tbl = table._tbl
                tblBorders = parse_xml(
                    f'<w:tblBorders {nsdecls("w")}><w:top w:val="none"/><w:left w:val="none"/>'
                    f'<w:bottom w:val="none"/><w:right w:val="none"/><w:insideH w:val="none"/>'
                    f'<w:insideV w:val="none"/></w:tblBorders>'
                )
                tbl.append(tblBorders)

                # Definiere die linke und rechte Zelle
                left_cell = table.cell(0, 0)
                right_cell = table.cell(0, 1)

                # --- FÜLLE DIE LINKE ZELLE (TEXT) ---
                # Hauptzeile für die Entität
                if entity['DATE']:
                    p_main = left_cell.add_paragraph(f"Entität {i} (Datum: {entity['DATE']}):")
                else:
                    p_main = left_cell.add_paragraph(f"Entität {i}:")
                self.set_font_properties(p_main)

                # Sammle Details für die Aufzählungsliste
                details_for_list = []
                if entity['SCREENAME']: details_for_list.append(f"Anzeigename: {entity['SCREENAME']}")
                if entity['VANITYNAME']: details_for_list.append(f"Nutzername: {entity['VANITYNAME']}")
                if entity['accountID']: details_for_list.append(f"Konto-ID: {entity['accountID']}")
                if entity['EMAIL']: details_for_list.append(f"E-Mail: {entity['EMAIL']}")
                if entity['PHONENUMBER']: details_for_list.append(f"Telefon: {entity['PHONENUMBER']}")
                
                status_info = []
                if entity['IMAGE']: status_info.append("Profilbild vorhanden")
                if entity['PRIVATE']: status_info.append("Privater Account")
                if status_info: details_for_list.append(f"Status: {', '.join(status_info)}")

                # Erstelle die Aufzählungspunkte in der linken Zelle
                for detail_text in details_for_list:
                    p_list = left_cell.add_paragraph(detail_text, style='List Bullet')
                    p_list.paragraph_format.left_indent = Inches(0.25)
                    self.set_font_properties(p_list)

                # --- FÜLLE DIE RECHTE ZELLE (BILD) ---
                script_dir = os.path.dirname(os.path.abspath(__file__))
                image_path = os.path.join(script_dir, 'images', f"{entity['entityID']}.jpg")

                if os.path.exists(image_path):
                    try:
                        # Füge das Bild in einem neuen Absatz in der rechten Zelle ein
